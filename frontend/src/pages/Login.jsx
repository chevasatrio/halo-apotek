...
function Login() {
  ...
  if (token && user?.role) {
    if (user.role === "pembeli") return <Navigate to="/pembeli" replace />;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!data?.token || !data?.user) {
        throw new Error("Format respons login tidak sesuai. Harus ada token & user.");
      }

      // 2) simpan dulu token + user dari response login
      login({ token: data.token, user: data.user });

      // 3) pastikan role ada (kalau tidak ada, ambil dari /api/user)
      let finalUser = data.user;
      if (!finalUser?.role) {
        const me = await api.get("/user"); // butuh Bearer token (interceptor Anda sudah set)
        finalUser = me.data;
        login({ token: data.token, user: finalUser }); // update context + localStorage
      }

      // 4) redirect sesuai role
      if (finalUser?.role === "pembeli") {
        navigate("/pembeli", { replace: true });
      } else {
        // default untuk selain pembeli: masuk dashboard
    ...
    <div className="min-h-screen bg-gradient-to-b from-sky-50 to-white flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-lg border p-8">
        ...
        <p className="mt-5 text-xs text-gray-500 text-center">
          Belum punya akun?{" "}
          <a className="text-sky-700 font-medium hover:underline" href="/register">
            Daftar
          </a>
        </p>
