...
export default function Hero() {
  ...
  const canUseBuyerFeatures = Boolean(token) && role === "pembeli";
  ...
  useEffect(() => {
    ...
    const loadCart = async () => {
      ...
      try {
        ...
        if (alive) setCartCount(items.length);
      } catch {
        if (alive) setCartCount(0);
      }
    };
  ...
  const goToObat = (useKeyword = true) => {
    const q = (search || "").trim();
    navigate("/pembeli/obat", { state: { q: useKeyword ? q : "" } });
  };
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-16">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2">
            {/* CART */}
            <Link
              to={canUseBuyerFeatures ? "/pembeli/cart" : "/login"}
              ...
              aria-label="Cart"
              title={canUseBuyerFeatures ? "Keranjang" : "Login dulu untuk buka keranjang"}
            >
              <IconCart />
              {canUseBuyerFeatures && cartCount > 0 && (
            ...
            <Link
  to="/pembeli/profile"
  className="hidden sm:flex items-center gap-2 bg-white/80 border border-slate-200 rounded-full px-3 py-1 shadow-sm hover:bg-sky-50 transition"
  aria-label="Profile"
  title="Profil"
>
